...
The {\tt websockets} package is a native HTML 5 Websocket implementation for R
that supports most of the draft IETF protocols in use by web browsers.
The {\tt websockets} package is especially well-suited to light weight
...
By ``light weight'' we mean that the library has few external dependencies and,
written mostly in R, is easily portable.  More significantly, {\tt websockets}
lets Javascript and other scripts embedded in web pages directly interact with
R, bypassing traditional middleware layers like .NET, Java, and web servers
normally used for such interaction.

The HTML 5 Websocket API is a modern socket-like communication protocol for the
web.  Note that the HTML 5 Websocket API is still under development and changes
frequently.  Some browsers may not enable Websockets by default (including
recent versions of Firefox), but there are usually simple methods to enable the
API.  Despite its developmental status, the API is presently widely supported:
...
All R/Websocket server applications use the following basic recpie:
\begin{enumerate}
...
The function takes 2 arguments, a network port
to listen on, and an optional function closure to service standard
HTTP requests. The {\tt create\_server} function returns an environment
that stores data associated with the newly created server.
So-called ``callback'' functions may be assigned that respond

...
The {\tt websockets} package supports multiple simultaneous websocket
connections.
...
is deleted, but are not sure about the implications for doing that yet.

\section{Tricks and miscellaneous notes}
We present a few more advanced and other miscellaneous notes in this
section.
\subsection{Binary data}
It is possible to exchange binary data over websockets, and the default

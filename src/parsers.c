  ...
  switch (wsi->parser_state)
    ...
    case WSI_TOKEN_HTTP:
      ...
      if (wsi->parser_state == WSI_TOKEN_GET_URI && c == ' ')
        {
          wsi->utf8_token[wsi->parser_state].token[wsi->
                                                   utf8_token[wsi->
                                                              parser_state].
                                                   token_len] = '\0';
      ...
      if (!wsi->utf8_token[WSI_TOKEN_UPGRADE].token_len)
        {
          ...
          debug ("Setting WSI_PARSING_COMPLETE " "from http headers\n");
          wsi->parser_state = WSI_PARSING_COMPLETE;
        }
...
libwebsocket_rx_sm (struct libwebsocket *wsi, unsigned char c)
{
  ...
  switch (wsi->lws_rx_parse_state)
    ...
    case LWS_RXPS_04_FRAME_HDR_LEN:
      ...
      switch (c)
        ...
        default:
          ...
          wsi->lws_rx_parse_state = LWS_RXPS_PAYLOAD_UNTIL_LENGTH_EXHAUSTED;
          break;
        }
      break;
    ...
    issue:
      if (wsi->protocol->callback)
        wsi->protocol->callback (wsi->protocol->owning_server,
                                 ...
                                 wsi->user_space,
                                 &wsi->
                                 rx_user_buffer[LWS_SEND_BUFFER_PRE_PADDING],
    ...
    case LWS_RXPS_SEEN_76_FF:
      ...
      debug ("Seen that client is requesting " "a v76 close, sending ack\n");
      buf[0] = 0xff;
      buf[1] = 0;
...
libwebsocket_client_rx_sm (struct libwebsocket *wsi, unsigned char c)
{
  ...
  switch (wsi->lws_rx_parse_state)
    ...
    case LWS_RXPS_04_FRAME_HDR_LEN:
      ...
      if (c & 0x80)
        {
          fprintf (stderr, "Frame has extensions set illegally 4\n");
          /* kill the connection */
          return -1;
        }
                    ...
                    size_t len, enum libwebsocket_write_protocol protocol)
{
      ...
      if ((protocol & 0xf) == LWS_WRITE_BINARY)
        {
          ...
          while (shift >= 0)
            {
              if (shift)
                buf[0 - pre + n] = ((len >> shift) & 127) | 0x80;
              else
                buf[0 - pre + n] = ((len >> shift) & 127);
              n++;
              shift -= 7;
            }
          break;
  ...
  if (wsi->mode == LWS_CONNMODE_WS_CLIENT && wsi->ietf_spec_revision >= 4)
    {
...
send_raw:
  ...
  if (protocol == LWS_WRITE_HTTP)
    {
      if (lws_issue_raw (wsi, (unsigned char *) buf - pre, len + pre + post))
        return -1;
  ...
  if (fd < 1)
    {
      p += sprintf (p, "HTTP/1.0 400 Bad\x0d\x0a"
                    "Server: libwebsockets\x0d\x0a" "\x0d\x0a");

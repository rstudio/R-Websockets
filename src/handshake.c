...
handshake_00 (struct libwebsocket *wsi)
{
  ...
  p += strlen ("HTTP/1.1 101 WebSocket Protocol Handshake\x0d\x0a"
               "Upgrade: WebSocket\x0d\x0a");
  strcpy (p, "Connection: Upgrade\x0d\x0a" "Sec-WebSocket-Origin: ");
  p += strlen ("Connection: Upgrade\x0d\x0a" "Sec-WebSocket-Origin: ");
...
handshake_0405 (struct libwebsocket *wsi)
{
  ...
  p += strlen ("HTTP/1.1 101 Switching Protocols\x0d\x0a"
               "Upgrade: WebSocket\x0d\x0a");
  strcpy (p, "Connection: Upgrade\x0d\x0a" "Sec-WebSocket-Accept: ");
  p += strlen ("Connection: Upgrade\x0d\x0a" "Sec-WebSocket-Accept: ");
  strcpy (p, accept_buf);
  p += accept_len;
          ...
          while (ext && ext->callback)
            {
              ...
              n =
                wsi->protocol->owning_server->protocols[0].
                callback (wsi->protocol->owning_server, wsi,
          ...
          !wsi->utf8_token[WSI_TOKEN_CONNECTION].token_len)
        {
          if (wsi->protocol->callback)
            (wsi->protocol->callback) (context, wsi,
                                       ...
                                       wsi->
                                       utf8_token[WSI_TOKEN_GET_URI].token,
                                       0);
